...
class weixinapi{
	public $_pre = "weixin_";
	public $dir = '';//如果网站不是在根目录请填写 如 shop/
	//搜索商品
	...
	function getFollowUserInfo($wxid){
		$sql = "select * from {$this->_pre}user where fake_id='{$wxid}'";
	...
	function bindUser($wxid,$email,$pwd,$username=''){
		...
		if($GLOBALS['db']->getOne("select ecuid from {$this->_pre}user where ecuid='{$user_id}'")){
			$GLOBALS['err']->add("该用户已经绑定过其他微信帐号！");
			return false;
		}
		$id = $GLOBALS['db']->getOne("select uid from {$this->_pre}user where fake_id='{$wxid}'");
		if($id > 0){
			$sql = "update {$this->_pre}user set ecuid={$user_id},isfollow=1 where fake_id='{$wxid}'";
		}else{
			$createtime = time();
			$createymd = date('Y-m-d');
			$sql = "insert into {$this->_pre}user (`ecuid`,`fake_id`,`createtime`,`createymd`,`isfollow`) 
	...
	function unBindUser($wxid){
		$sql = "update {$this->_pre}user set ecuid=0 where fake_id='{$wxid}'";
		$GLOBALS['db']->query($sql);
		return true;
	}
	//获取订单信息
	...
	function sendIntegral($wxid,$num=0,$key=""){
		...
		if($uid){
			if($key){
				$sql = "SELECT * FROM {$this->_pre}keywords where `key`='{$key}'";
				$rs = $GLOBALS['db']->getRow($sql);
				if($rs && $rs['jf_type']>0 && $rs['jf_num']>0){
					$num = $rs['jf_num'];
					if($rs['jf_type'] == 1){
						$maxNum = $GLOBALS['db']->getOne("SELECT sum(num) FROM {$this->_pre}jflog where fake_id='{$wxid}' and `key_id`='{$rs['id']}'");
						if($maxNum > 0) return false;
					}
					if($rs['jf_type'] == 2){
						$ymd = date('Y-m-d');
						$maxNum = $GLOBALS['db']->getOne("SELECT sum(num) FROM {$this->_pre}jflog where fake_id='{$wxid}' and `key_id`='{${$rs['id']}}' and createymd='{$ymd}'");
			...
			if($num > 0){
				...
				$createtime = time();
				$createymd = date('Y-m-d');
				$GLOBALS['db']->query("insert into {$this->_pre}jflog (`fake_id`,`jf_type`,`key_id`,`createtime`,`createymd`,`num`) value (
	...
	function followUser($wxid,$info=array()){
		...
		if($id>0){
			...
			$sql = "update {$this->_pre}user set from_id={$from_id},isfollow=1{$set} where uid={$id}";
		}else{
			$createtime = time();
			$createymd = date('Y-m-d');
			$sql = "insert into {$this->_pre}user (`ecuid`,`fake_id`,`createtime`,`createymd`,`isfollow`,`nickname`,`sex`,`country`,`province`,`city`,`access_token`,`expire_in`,`headimgurl`,`from_id`) 
	...
	function updateTokenExpire($wxid,$token){
		...
		$sql = "update {$this->_pre}user set access_token='$token',expire_in='$expire_in', where fake_id='{$wxid}'";
		$GLOBALS['db']->query($sql);
		return true;
	}
	//取消关注
	function unFollowUser($wxid){
		$GLOBALS['db']->query("update {$this->_pre}user set isfollow=0,expire_in=0 where fake_id='{$wxid}'");
		return true;
	}
	//保存用户输入的数据
	function saveMsg($content,$wxid,$type){
		if($content){
			...
			$sql = "insert into {$this->_pre}msg (`uid`,`fake_id`,`createtime`,`createymd`,`content`,`type`) 
	...
	function keywordsToKey($keys,&$diy_type){
		...
		$rs = $GLOBALS['db']->getRow("SELECT * FROM {$this->_pre}keywords where `keys` like '%{$keys}%' or `key`='{$keys}'");
		if($rs['key']){
			$GLOBALS['db']->query("update {$this->_pre}keywords set clicks=clicks+1 where id={$rs['id']}");
	...
	**/
	function pushToUserMsg($ecuid,$type="text",$msg=array(),$sendtime=0){
		$user = $GLOBALS['db']->getRow("select * from {$this->_pre}user where ecuid='{$ecuid}'");
		if($user && $user['fake_id']){
			...
			$createtime = time();
			$sql = "insert into {$this->_pre}corn (`ecuid`,`content`,`createtime`,`sendtime`,`issend`) 
	...
	function createTokenLoginUrl($wxid,$dir=''){
		...
		return $dir."weixin/redirect.php?token={$token}&t={$t}&wxid={$wxid}&url=";
	}
	//扫描登陆
	function scanLogin($content,$wxid){
		$login = $GLOBALS['db']->getRow ( "SELECT * FROM `weixin_login` WHERE `value` = '$content'" );
		if($login && $login['uid'] == 0 && $login['createtime']+600>time()){
			$uid = $this->isBindUser($wxid);
			if($uid){
				$GLOBALS['db']->query("UPDATE `weixin_login` SET `uid`=$uid WHERE `value` = '$content'");
				return true;
			}
		}
		return false;
	...
	function getAwardNum($aid){
		...
		if($act['type'] == 1){
			$ymd = date('Y-m-d');
			$sql = "SELECT count(1) FROM `weixin_actlog` WHERE `uid` = '$uid' and createymd='$ymd'";
		}else{
			$sql = "SELECT count(1) FROM `weixin_actlog` WHERE `uid` = '$uid'";
		}
	...
	function doAward($aid){
		...
		$GLOBALS['db']->query("INSERT INTO `weixin_actlog` (uid,aid,class_name,createymd,createtime,code,issend) 
	...
	function randAward($aid){
		...
		$actList = $GLOBALS['db']->getAll ( "SELECT title,lid,randnum,awardname,num FROM `weixin_actlist` where aid=$aid and isopen=1 and num>num2 order by num desc" );
		if($actList){
			foreach($actList as $v){
				if(intval(rand(1,10000)) <= $v['randnum']*100){
					$v['code'] = uniqid();
					$GLOBALS['db']->query("update `weixin_actlist` set num2=num2+1 where lid={$v['lid']}");
					return $v;
				}
			}
		}
	...
	private function checkAward($aid){
		$act = $GLOBALS['db']->getRow ( "SELECT * FROM `weixin_act` where aid=$aid" );
	...
	function userSign($wxid){
		...
		$ymd = date('Y-m-d',time());
		if($info['ecuid'] > 0){
			$conf = $GLOBALS['db']->getRow ( "SELECT * FROM `weixin_signconf` where cid=1 and startymd<='{$ymd}' and endymd>='{$ymd}'" );
			if(!$conf){
				$GLOBALS['err']->add("没有开启签到");
				return false;
			}
			$issign = $GLOBALS['db']->getOne("SELECT wxid FROM `weixin_sign` where wxid={$info['uid']} and signymd='{$ymd}'");
			if($issign){
				$GLOBALS['err']->add("您今天已经签过到了");
				return false;
			}
			$ymd2 = date('Y-m-d',time()-86400);//检查昨天是否签到
			$issign = $GLOBALS['db']->getOne("SELECT sid FROM `weixin_sign` where wxid={$info['uid']} and signymd='{$ymd2}'");
			if($issign){
			...
			$GLOBALS['db']->query("insert into weixin_sign (`wxid`,`signtime`,`signymd`) value ('{$info['uid']}','{$nowtime}','{$ymd}')");
			$GLOBALS['db']->query("update weixin_user set sign_num=$sign_num where uid='{$info['uid']}'");
			return $num;
		}else{
