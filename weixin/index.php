...
$weixinconfig = $db->getRow ( "SELECT * FROM `weixin_config` WHERE `id` = 1" );
//多微信帐号支持
$id = intval($_GET['id']);
if($id > 0){
	$otherconfig = $db->getRow ( "SELECT * FROM `weixin_config` WHERE `id` = $id" );
	if($otherconfig){
...
if ($event['event'] == "SCAN"){
	...
	$res = $db->getRow("select * from weixin_qcode where id='$content'");
	if($res){
		if($res['type'] == 1){
			$goodsInfo = $db->getRow("select * from ".$ecs->table('goods')." where goods_id='{$res['content']}'");
			...
			$artInfo = $db->getRow("select * from ".$ecs->table('article')." where article_id='{$res['content']}'");
			$newsData[0]['Title'] = $artInfo['title'];
